...
const TaxCodes = () => {
  const [taxCodes, setTaxCodes] = useState([
    { id: 1, code: 'GST18', name: 'GST 18%', rate: 18, description: 'Goods and Services Tax at 18%', applicableFor: ['Services', 'Electronics'] },
    { id: 2, code: 'GST12', name: 'GST 12%', rate: 12, description: 'Goods and Services Tax at 12%', applicableFor: ['Processed Foods', 'Medicines'] },
    { id: 3, code: 'GST5', name: 'GST 5%', rate: 5, description: 'Goods and Services Tax at 5%', applicableFor: ['Essential Goods', 'Transportation'] },
    { id: 4, code: 'GST0', name: 'GST 0%', rate: 0, description: 'Goods and Services Tax at 0%', applicableFor: ['Exports', 'Essential Services'] },
  ]);
  ...
  // Filter tax codes based on search term and rate
  const filteredTaxCodes = taxCodes.filter(taxCode => {
    ...
    const matchesRate = filterRate === '' || 
      taxCode.rate.toString() === filterRate;
    return matchesSearch && matchesRate;
  });

  const rateOptions = ['0', '5', '12', '18', '28'];
  ...
  const handleSubmit = (e) => {
    ...
    if (editingTaxCode) {
      setTaxCodes(codes => 
        codes.map(code => 
          code.id === editingTaxCode.id ? { ...code, ...formData, rate: parseFloat(formData.rate) } : code
        )
      );
    } else {
      setTaxCodes(codes => [...codes, { id: Date.now(), ...formData, rate: parseFloat(formData.rate) }]);
    }
    setShowForm(false);
    setEditingTaxCode(null);
    setFormData({ code: '', name: '', rate: '', description: '', applicableFor: [] });
  };

  const handleEdit = (taxCode) => {
    setEditingTaxCode(taxCode);
    setFormData({ ...taxCode, rate: taxCode.rate.toString() });
    setShowForm(true);
  };

  const handleDelete = (id) => {
    setTaxCodes(codes => codes.filter(code => code.id !== id));
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Manage your tax codes for financial transactions</p>
            </div>
          </div>
          <div className="flex items-center">
      ...
      <div className="space-y-6">
        ...
        <div className="mb-6 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label htmlFor="filterRate" className="block text-xs font-medium text-gray-500 mb-1">Tax Rate</label>
              <select
                id="filterRate"
              ...
              {filteredTaxCodes.length > 0 ? (
                ...
                <tr>
                  <td colSpan="6" className="px-6 py-8 text-center text-gray-500">
                    No tax codes found matching your filters.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                Showing <span className="font-medium">1</span> to <span className="font-medium">{Math.min(filteredTaxCodes.length, 10)}</span> of{' '}
                <span className="font-medium">{filteredTaxCodes.length}</span> results
              </p>
            </div>
            <div>
            ...
            <div className="mt-3">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={formData.applicableFor.join(', ')}
                    onChange={(e) => setFormData({...formData, applicableFor: e.target.value.split(', ').filter(item => item.trim())})}
